...
- Before each commit:
  ...
  - Run `pre-commit run --all-files`. This runs the hooks specified in `.pre-commit-hooks.yaml` and configured in
    `pyproject.toml`, for example for linting, formatting, type checking, etc. Change and iterate until all hooks pass.
  - Repeat all of the above steps if at least one change was made during the current repetition.
