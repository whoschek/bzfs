...
dst_hosts = {
    "testdst01": ["", "onsite"]
}  # host named "testdst01" will receive replicas for snapshots with target="" or target="onsite"
...
# }
dst_root_datasets = {
    "testdst01": "",  # Empty string means 'Don't prepend a prefix' (for safety, the hostname must always be in the dict)
}
...
# }
src_snapshot_plan = {
    org: {"onsite": {"secondly": 1, "minutely": 40, "hourly": 36, "daily": 31, "weekly": 0, "monthly": 0, "yearly": 0}}
}
...
# workers = 1  # aka max_workers = 1  # run jobs sequentially for easier debugging
...
# extra_args += ["--create-bookmarks=all"]  # this is already the default anyway
...
#     # "--publish=2222:22",
#     f"--mount=type=bind,src={home_dir}/.ssh,dst=/root/.ssh,readonly",  # inherits auth from host to container (read-only)
