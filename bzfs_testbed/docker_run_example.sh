...
# Assumes that the $BZFS_DOCKER_IMAGE is available, for example generated by `sudo ./docker_image.sh`.
set -eo pipefail
...
if true; then
    container_exec /bzfs/bzfs_testbed/bzfs_job_testbed.py --create-src-snapshots --replicate \
        --ssh-src-port=2222 \
        --ssh-dst-port=2222
fi
