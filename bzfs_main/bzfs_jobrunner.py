...
def argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=f"""
This companion program wraps [bzfs](README.md) for periodic snapshot creation, replication, pruning, and monitoring across
N source hosts and M destination hosts, using one shared fleet-wide [jobconfig](bzfs_testbed/bzfs_job_testbed.py) script.
...
A separate cron job on each source host and each destination host runs `{PROG_NAME}` periodically to alert the user if the
latest or oldest snapshot is somehow too old (via --monitor-src-snapshots and --monitor-dst-snapshots).
The frequency of each activity can differ. Typical intervals range from N milliseconds to years.
...
replicate from selected source hosts to selected destination hosts.
Each `{PROG_NAME}` invocation runs all enabled actions for the final effective values of `--src-hosts` and `--dst-hosts`,
after applying the `--src-host` and `--dst-host` filters.
...
The daemon processes work like non-daemon processes except that they loop, handle time events, and sleep between events.
They exit after the interval specified by `--daemon-lifetime` (for example, 86400 seconds). They are then restarted by
`cron`, or earlier if they fail. While an existing daemon is still running, `cron` may attempt to start another one.
This is harmless because that extra process exits immediately with a message like this:
