...
# constants:
__version__: Final[str] = "1.26.0.dev0"
...
required there. {PROG_NAME} can run as root or as a non-root user via sudo or delegated `zfs allow`
permissions.
...
logs [various summary stats](https://github.com/whoschek/bzfs/blob/main/bzfs_docs/compare-snapshot-lists-example.log),
...
""")
    ...
    parser.add_argument(
        ...
        help="During replication, handle source datasets that select no snapshots (and no relevant bookmarks) "
             ...
             "--recursive and destination dataset does not exist or is inconsistent. Otherwise skip to the next dataset.\n\n"
             "c) 'continue': Continue processing the next dataset, including descendants. If an inconsistent "
             "destination dataset prevents processing selected descendants, fail with an error; --skip-on-error "
             "determines whether processing continues elsewhere. "
    ...
    parser.add_argument(
        ...
        help="During replication and deletion, if an error is not retryable, or --retries has been exhausted, "
             ...
             "c) 'dataset' (default): Same as 'tree' except if the destination dataset already exists and is not "
             "inconsistent, skip to the next dataset instead.\n\n"
             ...
             "if the destination already contains tank/user1 and that dataset is not inconsistent. Otherwise "
             "processing continues with tank/user2. "
    ...
    parser.add_argument(
        ...
        help="Do nothing if the --delete-dst-snapshots option is missing. Otherwise, after successful "
             ...
             "*Note:* Use --delete-dst-snapshots=bookmarks to delete non-temporary bookmarks instead of snapshots, in "
             "which case no snapshots are selected and the --{include|exclude}-snapshot-* filter options treat bookmarks "
    ...
    parser.add_argument(
        "--force-delete-dst-tmp-bookmarks", action="store_true",
        help=argparse.SUPPRESS)
    parser.add_argument(
        ...
        help="This flag indicates that --delete-dst-snapshots=snapshots shall check the source dataset only for "
             ...
             "source dataset. Similarly, it also indicates that --delete-dst-snapshots=bookmarks shall check the source "
             "dataset only for a bookmark with the same GUID, and ignore whether a snapshot with the same GUID is present "
    ...
    parser.add_argument(
        ...
        help="Do nothing if the --compare-snapshot-lists option is missing. Otherwise, after successful replication "
             ...
             "The --compare-snapshot-lists option also directly logs [various summary stats]"
             "(https://github.com/whoschek/bzfs/blob/main/bzfs_docs/compare-snapshot-lists-example.log), "
             "such as the metadata of the latest common snapshot, latest snapshots and oldest snapshots, as well as the "
             "time diff between the latest common snapshot and latest snapshot only in src (and only in dst), as well as "
             "how many src snapshots and how many GB of data are missing on dst, etc.\n\n"
             "*Note*: By default, if the source ZFS pool supports bookmarks, source bookmarks also participate in the "
             "comparison. A source bookmark and destination snapshot with the same GUID are considered to be contained "
             "in both locations, even if the corresponding source snapshot has already been deleted. Unmatched temporary "
             f"bookmarks managed by {PROG_NAME} remain hidden. Specify --no-use-bookmark to compare snapshots only.\n\n"
    ...
    parser.add_argument(
        ...
        help=f"For increased safety, {PROG_NAME} replication behaves as follows wrt. ZFS bookmark creation, if it is "
             ...
             "* `all` (default): Selects every source snapshot that will be sent during each 'zfs send' operation. "
             "This increases safety at the expense of a little performance.\n\n"
             "* `hourly`: Selects each hourly, daily, weekly, monthly and yearly source snapshot that will be sent.\n\n"
             "* `minutely` and `secondly`: Same as `hourly` except that it also selects minutely and secondly snapshots, "
             "respectively.\n\n"
             "* `none`: No bookmark is created.\n\n"
             "For every mode except `none`, the snapshot of a full send and the final snapshot of incremental "
             "replication are always selected as they establish the initial and latest common bases, respectively.\n\n"
             f"For each selected source snapshot, {PROG_NAME} creates a destination-specific temporary bookmark before "
             "starting its 'zfs send/receive' operation. After the operation succeeds, it renames the temporary bookmark "
             "to a finalized bookmark and deletes any obsolete temporary bookmarks for that destination. Both temporary "
             "and finalized bookmarks can serve as sources for future incremental replication once the corresponding "
             f"snapshot exists on the destination. This guarantees continuity even if the {PROG_NAME} process is killed "
             "after 'zfs send/receive' succeeds but before bookmark finalization.\n\n"
             ...
             "has been successfully replicated. This way you can bookmark a snapshot on the source dataset, send the "
             "snapshot to another host, then delete the snapshot from the source dataset to save disk space, and then "
             "still incrementally send the next upcoming snapshot from the source dataset to the other host by "
             ...
             f"By convention, a finalized bookmark created by {PROG_NAME} has the same name as its corresponding "
             "snapshot, except the name also contains the snapshot GUID for uniqueness. Destination-specific temporary "
             "bookmarks use names that begin with `.TMPBZFS.`; this namespace is reserved for internal use. Do not create "
             f"or delete such temporary bookmarks because {PROG_NAME} automatically manages and garbage collects them.\n\n"
             ...
             "As an example starting point, here is a command that deletes all non-temporary bookmarks older than "
             "90 days, but retains the latest 200 non-temporary bookmarks (per dataset) regardless of creation time: "
    ...
    parser.add_argument(
        ...
        help=f"For increased safety, in normal replication operation {PROG_NAME} replication also looks for bookmarks "
             ...
             "destination dataset, if it is auto-detected that the source ZFS pool supports bookmarks. "
             ...
             "number and creation time as the snapshot. Apart from the internal `.TMPBZFS.` namespace described above, "
             "you can create, delete and prune bookmarks any way you like, as "
             f"{PROG_NAME} (without --no-use-bookmark) will happily work with whatever bookmarks currently exist, if any.\n\n")
    ...
    # see https://web.archive.org/web/20251011105141if_/https://gbe0.com/posts/linux/server/benchmark-ssh-ciphers/
    ...
    parser.add_argument(
        "--r2r", choices=["off", "pull", "push"], default="off",
        help="For remote-to-remote replication, controls whether the `zfs send` stream is relayed by the initiator "
             "or transferred directly between source and destination in order to improve the throughput of bulk data "
             "transfers (default: %(default)s). This option has no effect unless both replication endpoints are remote.\n\n"
             "* `off`: Keeps existing pull-push behavior where the initiator acts as an intermediary that relays the "
             "`zfs send` stream between source and destination, which can become a central bandwidth bottleneck.\n\n"
             "Example: ssh alice@srchost 'zfs send ...' | ssh bob@dsthost 'zfs receive ...'\n\n"
             "* `pull`: Tells the destination host to pull the `zfs send` stream directly from the source host. Requires "
             "`sh` and `ssh` on the destination host, plus SSH setup such that the destination user@host can `ssh` into "
             "the source host.\n\n"
             "Example: ssh bob@dsthost \"sh -c 'ssh alice@srchost zfs send ... | zfs receive ...'\"\n\n"
             "* `push`: Tells the source host to push the `zfs send` stream directly to the destination host. Requires "
             "`sh` and `ssh` on the source host, plus SSH setup such that the source user@host can `ssh` into the "
             "destination host.\n\n"
             "Example: ssh alice@srchost \"sh -c 'zfs send ... | ssh bob@dsthost zfs receive ...'\"\n\n"
             "*Note:* Orchestration still runs on the initiator; only the bulk data path changes. It is recommended to "
             "also set `--ssh-src-user` and `--ssh-dst-user` explicitly in order to avoid potential confusion about which "
             "SSH user account performs the nested remote-to-remote operation.\n\n"
             f"*Note:* If the required nested `sh`/`ssh` do not exist, {PROG_NAME} falls back to `--r2r=off`. "
             "`--r2r=pull` falls back to `--r2r=off` if `--ssh-src-config-file` is set to a non-empty value other than "
             "`none`, and `--r2r=push` falls back to `--r2r=off` if `--ssh-dst-config-file` is set to a non-empty value "
             "other than `none`.\n\n"
