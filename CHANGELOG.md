...
- [bzfs] fix: Reject a non-raw `zfs send` of an encrypted dataset with an unloaded source key, even when the
  `zfs receive` succeeds. Also fix similar "cannot send '<dataset>': <reason>" errors where reason is one of ["source
  key must be loaded", "permission denied", "Input/output error", "Invalid argument", "unsupported version or feature",
  "incremental source (@<snapshot>) does not exist"].
...
- [bzfs] Make it possible to use `zfs send -R -s` aka sending a recursive stream package. Example usage:
  `bzfs --recursive --zfs-send-program-opts='--raw --compressed -R -s' <src_root_dataset> <dst_root_dataset>`
...
- [bzfs] Log each created snapshot for better diagnostics.
- [bzfs] Productionize [`markdown_from_argparse.py`](README_markdown_from_argparse.md) CLI util.
- [bzfs] Also run nightly CI tests on ubuntu-26.04 and FreeBSD-15.1.
- [bzfs] Also run nightly CI tests on zfs-2.4.3 and zfs-2.3.8 and zfs-2.2.10.
- For the full list of changes, see https://github.com/whoschek/bzfs/compare/v1.22.0...v1.23.0
