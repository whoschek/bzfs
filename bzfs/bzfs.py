...
""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument(
        ...
        help="SRC_DATASET: "
             ...
             f"*Performance Note:* {prog_name} automatically replicates multiple datasets in parallel (and replicates "
             "snapshots serially within a dataset). All children of a dataset may be processed in parallel. For "
             "consistency, processing of a dataset only starts after processing of all its ancestor datasets has "
             "completed. Further, when a thread is ready to start processing another dataset, it chooses the "
             "'smallest' dataset wrt. case-insensitive sort order from the datasets that are currently available for "
             "start of processing. Initially, only the roots of the selected dataset subtrees are available for start "
             "of processing. The degree of parallelism is configurable with the --threads option (see below).\n\n")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="The maximum number of threads to use for parallel operations; can be given as a positive integer, "
             ...
             "applies to snapshot replication, --delete-dst-snapshots, --delete-empty-dst-datasets, and "
            ...
            while submit_datasets():
                ...
                for done_future in done_futures:
                    ...
                    if no_skip and children:  # make child datasets available for start of processing ...
                        for name, grandchildren in children.items():  # as processing of parent has now completed
                            name = f"{dataset}/{name}"
