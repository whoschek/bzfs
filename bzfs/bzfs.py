...
""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument(
        "--no-send-size-estimate", action="store_true",
...
class Params:
    def __init__(
        ...
        self.no_send_size_estimate: bool = args.no_send_size_estimate
...
class Job:
    ...
    def refresh_ssh_connection_if_necessary(self, r: Remote, conn) -> None:
        ...
        if r.ssh_user_host != "":
            ...
            if not r.reuse_ssh_connection:
                return
            with conn.lock:
                # performance: reuse ssh connection for low latency startup of frequent ssh invocations
                # see https://www.cyberciti.biz/faq/linux-unix-reuse-openssh-connection/
                if time.time_ns() - conn.last_refresh.value < r.control_persist_limit:
                    return  # ssh master is still alive, reuse its conn (this is the common case & the ultra-fast path)
                ...
                # 'ssh -S /path/socket -O check' doesn't talk over the net, hence is still a low latency fast path
                ...
                else:  # ssh master is not alive; start a new ssh master:
    ...
    def estimate_send_size(self, remote: Remote, dst_dataset: str, recv_resume_token: str, *items) -> int:
        ...
        p, log = self.params, self.params.log
        if p.no_send_size_estimate or self.is_solaris_zfs(remote):
