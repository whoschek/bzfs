...
--include-snapshot-regex '.*_daily' --include-snapshot-times-and-ranks '0..0' 'except latest 7'
...
--include-snapshot-regex '.*_daily' --include-snapshot-times-and-ranks '0..0' 'except latest 200'
...
""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument(
        ...
        help="This option takes as input parameters a time range filter and an optional rank range filter. It "
             ...
             "optional word 'except' (followed by an optional space), followed by the word 'oldest' or 'latest', followed "
             "by a non-negative integer, followed by an optional '%%' percent sign. A rank range consists of a lower rank, "
             "followed by a '..' separator, followed by a higher rank. "
             ...
             "* 'except latest 10%%' aka 'oldest 90%%' aka 'oldest 0..oldest 90%%' (include all snapshots except the latest "
             "10%% of all snapshots)\n\n"
             ...
             "* 'except oldest 90' aka 'oldest 90..oldest 100%%' (include all snapshots except the oldest 90 snapshots)\n\n"
             "* 'except latest 90' aka 'latest 90..latest 100%%' (include all snapshots except the latest 90 snapshots)\n\n"
             "* 'latest 1' aka 'latest 0..latest 1' (include the latest snapshot)\n\n"
             "* 'except latest 1' aka 'latest 1..latest 100%%' (include all snapshots except the latest snapshot)\n\n"
             "* 'oldest 2' aka 'oldest 0..oldest 2' (include the oldest 2 snapshots)\n\n"
             "* 'except oldest 2' aka 'oldest 2..oldest 100%%' (include all snapshots except the oldest 2 snapshots)\n\n"
    ...
    parser.add_argument(
        ...
        help="Before replication, delete destination ZFS snapshots that are more recent than the most recent common "
             "snapshot selected on the source ('conflicting snapshots'), via 'zfs rollback'. Instead, abort with an error "
             "if no common snapshot is selected but the destination already contains a snapshot.\n\n")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"For increased safety, in normal operation {prog_name} behaves as follows wrt. ZFS bookmark creation, "
             ...
             "--delete-dst-snapshots=bookmarks --include-snapshot-times-and-ranks '0..0' 'except latest 200' "
             "--include-snapshot-times-and-ranks '*..90 days ago'`\n\n")
    parser.add_argument(
...
class TimeRangeAndRankRangeAction(argparse.Action):
    ...
    def parse_rankranges(parser, values, option_string=None) -> List[RankRange]:
        def parse_rank(spec):
            spec = spec.strip()
            match = re.fullmatch(r"(except)?\s*(oldest|latest)\s*(\d+)%?", spec)
            if not match:
        ...
        for value in values:
            ...
            if ".." in value:
                ...
                if lo[0] or hi[0]:
                    # Example: 'except latest 90..except latest 95' or 'except latest 90..latest 95'
                ...
                if is_except:
                    if is_percent:
                        # 'except latest 10%' aka 'oldest 90%' aka 'oldest 0..oldest 90%'
                        # 'except oldest 10%' aka 'latest 90%' aka 'latest 0..oldest 90%'
                        negated_kind = "oldest" if kind == "latest" else "latest"
                        lo = parse_rank(f"{negated_kind}0")
                        hi = parse_rank(f"{negated_kind}{100-num}%")
                    else:
                        # 'except latest 90' aka 'latest 90..latest 100%'
                        # 'except oldest 90' aka 'oldest 90..oldest 100%'
