...
class Job:
    ...
    def run_zfs_send_receive(
        ...
        pool_name = DEDICATED if p.dedicated_tcp_connection_per_zfssend else SHARED
        src_conn_pool: ConnectionPool = p.connection_pools["src"].pool(pool_name)
        src_conn: Connection = src_conn_pool.get_connection()
        dst_conn_pool: ConnectionPool = p.connection_pools["dst"].pool(pool_name)
