...
class TestReplication(AbstractTestCase):
    ...
    def test_native_send_errors_with_successful_receive_are_retryable(self) -> None:
        ...
        diagnostics, optional warning prefixes, multiple diagnostics, CRLF, and an unterminated final line so reason capture
        stays independent of surrounding output.
        """
        mbuffer_status = "in @  0.0 kiB/s, out @  0.0 kiB/s, 20.0 MiB total, buffer 100% full"
        cases = [
            ("warning: ", "source key must be loaded", "\n"),
            ("warning: ", "permission denied", "\n"),
            ("warning: ", "Input/output error", "\nwarning: cannot send 'pool/src@s3': Invalid argument\n"),
            ("", "Invalid argument", "\r\n"),
            ("warning: ", "unsupported version or feature", ""),
            ("", "dataset key must be loaded", "\n"),
            ("warning: ", "incremental source (@s1) does not exist", "\n"),
            (mbuffer_status + "warning: ", "permission denied", "\n"),
            (mbuffer_status, "source key must be loaded", "\n"),
            ("foo\n" + mbuffer_status, "source key must be loaded", "\n" + "foo\n"),
        ]
        ...
        for prefix, reason, suffix in cases:
            with self.subTest(prefix=prefix, reason=reason):
                ...
                stderr = f"TIME        SENT   SNAPSHOT\n{prefix}cannot send 'pool/src@s2': {reason}{suffix}"
    ...
    def test_decode_resume_token_validates_source_dataset_and_name(self) -> None:
        ...
        def decode(stdout: str) -> str:
            with patch.object(job, "run_ssh_command", return_value=stdout):
                return _decode_resume_token(job, "token", "src/pool", "dst/pool", {"2748": "src/pool@s1"})
